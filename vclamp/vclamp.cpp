...
double* pp_property_ptr(Object* pp, const char* name) {
...
int main(void) {
    ...
    *((int64_t*) dlsym(handle, "nrn_nobanner_")) = 1;
    ...
    auto v_vec = hoc_newobj1(sym, 0);
    hoc_pushpx(nrn_rangepointer(axon, hoc_lookup("v"), 0.5));
    tempobj = call_object_method_that_returns_object(v_vec, "record", 1);
    // note: Vector.record returns an object, so we must unref it
    hoc_obj_unref(tempobj);

    /***************************
     * adding a VClamp at axon(0.5)
     **************************/
    nrn_pushsec(axon);
    hoc_pushx(0.5);
    auto vclamp = hoc_newobj1(hoc_lookup("VClamp"), 1);
    nrn_sec_pop();

    auto vclamp_amp = pp_property_ptr(vclamp, "amp");
    auto vclamp_dur = pp_property_ptr(vclamp, "dur");
    ...
    sym = hoc_table_lookup("printf", v_vec->ctemplate->symtable);
    assert(sym);
    call_ob_proc(v_vec, sym, 0);
